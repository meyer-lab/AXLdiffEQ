...
ffnonSpat2 = @(x) cLib_diff_profile_pYavg (t, 359.46, 0.0581, Gass([-100 x(2) x(3)]), ones(1,14)*100, 1, 1, frac);
...
for ii = 1:length(A)
    B(ii) = ffSpat([A(ii) GasConc shapeParam]);
    B2(ii) = ffnonSpat([A(ii) GasConc shapeParam]);
    B3(ii) = ffnonSpat2([A(ii) GasConc shapeParam]);
