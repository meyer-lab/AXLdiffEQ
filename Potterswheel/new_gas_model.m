...
if ~isempty(strfind(postTag,'longT'))
    ...
    m = pwAddY(m,'GASamt',['scale_gas*(A1+A2+2*A12+D1+2*D2 + ' mat2str(EndoMemFrac) '*(A1i+A2i+2*A12i+D1i+2*D2i+Gasi))']);
...
m = pwAddK(m,'B1',1.2,'global',0.0006,0.6); % Limits of 10^4 to 10^8
m = pwAddK(m,'B2',0.30673,'global',0.0006,0.6);
m = pwAddK(m,'U1',0.042,'global');
