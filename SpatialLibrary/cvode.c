...
int CVodeRootInit(void *cvode_mem, int nrtfn, CVRootFn g)
{
  ...
  glo = NULL;
  glo = (double *) malloc((size_t) nrt*sizeof(double));
  if (glo == NULL) {
  ...
  ghi = NULL;
  ghi = (double *) malloc((size_t) nrt*sizeof(double));
  if (ghi == NULL) {
    free(glo); glo = NULL;
    CVProcessError(cv_mem, CV_MEM_FAIL, "CVODE", "CVodeRootInit", MSGCV_MEM_FAIL);
    return(CV_MEM_FAIL);
  ...
  grout = NULL;
  grout = (double *) malloc((size_t) nrt*sizeof(double));
  if (grout == NULL) {
    free(glo); glo = NULL;
    free(ghi); ghi = NULL;
    CVProcessError(cv_mem, CV_MEM_FAIL, "CVODE", "CVodeRootInit", MSGCV_MEM_FAIL);
  ...
  iroots = NULL;
  iroots = (int *) malloc((size_t) nrt*sizeof(int));
  if (iroots == NULL) {
    free(glo); glo = NULL; 
    free(ghi); ghi = NULL;
    free(grout); grout = NULL;
  ...
  rootdir = NULL;
  rootdir = (int *) malloc((size_t) nrt*sizeof(int));
  if (rootdir == NULL) {
    free(glo); glo = NULL; 
    free(ghi); ghi = NULL;
    free(grout); grout = NULL;
          ...
          double *tret, int itask)
{
  ...
  if (nst > 0) {
    ...
    /* Estimate an infinitesimal time interval to be used as
       ...
       and step size) */
    troundoff = FUZZ_FACTOR*uround*(fabs(tn) + fabs(h));
    ...
    if ( itask == CV_ONE_STEP && fabs(tn - tretlast) > troundoff ) {
      tretlast = *tret = tn;
      N_VScale(ONE, zn[0], yout);
      return(CV_SUCCESS);
    }
...
int CVodeGetDky(void *cvode_mem, double t, int k, N_Vector dky)
{
  ...
  tfuzz = FUZZ_FACTOR * uround * (fabs(tn) + fabs(hu));
...
static int CVHin(CVodeMem cv_mem, double tout)
{
  ...
  sign = (tdiff > ZERO) ? 1 : -1;
  tdist = fabs(tdiff);
  tround = uround * MAX(fabs(tn), fabs(tout));
                       ...
                       double alpha0_hat, double xi_inv, double xistar_inv)
{
  ...
  A2 = ONE + q * A1;
  tq[2] = fabs(A1 / (alpha0 * A2));
  tq[5] = fabs(A2 * xistar_inv / (l[q] * xi_inv));
  if (qwait == 1) {
    if (q > 1) {
      C = xistar_inv / l[q];
      A3 = alpha0 + ONE / q;
    ...
    Cppinv = (ONE - A6 + A5) / A2;
    tq[3] = fabs(Cppinv / (xi_inv * (q+2) * A5));
  }
  tq[4] = nlscoef / tq[2];
}
...
static int CVNlsFunctional(CVodeMem cv_mem)
{
  ...
  loop {
    ...
    N_VLinearSum(1.0, tempv, -ONE, acor, acor);
...
static int CVNlsNewton(CVodeMem cv_mem, int nflag)
{
  ...
  if (setupNonNull) {      
    callSetup = (nflag == PREV_CONV_FAIL) || (nflag == PREV_ERR_FAIL) ||
      (nst == 0) || (nst >= nstlp + MSBP) || (fabs(gamrat-ONE) > DGMAX);
  } else {  
    crate = ONE;
    callSetup = FALSE;
  }
 ...
 * returning the value TRY_AGAN. (In this case, CVNlsNewton must set 
...
static int CVNewtonIteration(CVodeMem cv_mem)
{
  ...
  loop {
    ...
    b = tempv;
    retval = cv_mem->cv_lsolve(cv_mem, b, ewt, y, ftemp); 
    nni++;
    ...
    if (retval > 0) { 
      if ((!jcur) && (setupNonNull)) return(TRY_AGAN);
                         ...
                         int *ncfPtr)
{
  ...
  if ((fabs(h) <= hmin*ONEPSM) || (*ncfPtr == maxncf)) {
 ...
 *
 *   - otherwise, set *nflagPtr to PREV_ERR_FAIL, and return TRY_AGAN. 
 *
 */
                                ...
                                double saved_t, int *nefPtr, double *dsmPtr)
{
  ...
  *nflagPtr = PREV_ERR_FAIL;
  ...
  if ((fabs(h) <= hmin*ONEPSM) || (*nefPtr == maxnef)) return(CV_ERR_FAILURE);
  ...
  if (*nefPtr <= MXNEF1) {
    ...
    eta = MAX(ETAMIN, MAX(eta, hmin / fabs(h)));
...
static int CVRcheck1(CVodeMem cv_mem)
{
  ...
  tlo = tn;
  ttol = (fabs(tn) + fabs(h))*uround*HUN;
...
static int CVRootfind(CVodeMem cv_mem)
{
  ...
  if (!sgnchg) {
    ...
    for (i = 0; i < nrtfn; i++) {
      iroots[i] = 0;
      if(!gactive[i]) continue;
      if (fabs(ghi[i]) == ZERO) iroots[i] = glo[i] > 0 ? -1:1;
    }
    return(RTFOUND);
  }
  ...
  loop {                                    /* Looping point */
    ...
    if (fabs(tmid - tlo) < HALF*ttol) {
      fracint = fabs(thi - tlo)/ttol;
      fracsub = (fracint > FIVE) ? TENTH : HALF/fracint;
      tmid = tlo + fracsub*(thi - tlo);
    }
    if (fabs(thi - tmid) < HALF*ttol) {
      fracint = fabs(thi - tlo)/ttol;
      fracsub = (fracint > FIVE) ? TENTH : HALF/fracint;
    ...
    for (i = 0;  i < nrtfn; i++) {
      if(!gactive[i]) continue;
      if (fabs(grout[i]) == ZERO) {
        if(rootdir[i]*glo[i] <= ZERO) {
          zroot = TRUE;
        }
      } else {
        if ( (glo[i]*grout[i] < ZERO) && (rootdir[i]*glo[i] <= ZERO) ) {
          gfrac = fabs(grout[i]/(grout[i] - glo[i]));
          if (gfrac > maxfrac) {
            sgnchg = TRUE;
            maxfrac = gfrac;
            imax = i;
  ...
  for (i = 0; i < nrtfn; i++) {
    ...
    iroots[i] = 0;
    if(!gactive[i]) continue;
    if ( (fabs(ghi[i]) == ZERO) && (rootdir[i]*glo[i] <= ZERO) )
