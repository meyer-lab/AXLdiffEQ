...
@pymc.potential
def psi_diff(x_in = diffMeas):
    return -(100/x_in);
    
@pymc.potential
def psi_i(x_in = errMeas):
    return pymc.distributions.t_like(x_in, 37);



if __name__ == '__main__':
    # Instantiate model
    M = pymc.MCMC([x, errMeas, psi_i],db='pickle')
    # Sample
    M.use_step_method(pymc.AdaptiveMetropolis, x);
    M.sample(iter=int(2E6), burn=int(1E6), thin=20)
